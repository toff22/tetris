...
@click.command(help='Update env variables of a a container version')
@click.option('--profile', '-p', help=pyke.helptxt.PROFILE, callback=pyke.select_profile, shell_complete=pyke.auth.get_profile_names)
@click.option('--container', '-c', prompt=True)
@click.option('--version', '-v', prompt=True, help='The version ID or the version-number')
@click.option('--env', '-e', default=None, multiple=True, help="The environment variables to add to the docker image when it is run -- Syntax: envVar=value")
def update_variables(**kwargs):
  behavior.ContainerVersionBehavior(**kwargs).update_variables()
...
commands.microservice_version.add_command(update_variables)
commands.widget_version.add_command(update_variables)
commands.app_builder_version.add_command(update_variables)
commands.asset_version.add_command(update_variables)